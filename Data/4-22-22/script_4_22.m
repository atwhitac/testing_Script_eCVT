...
lastAverage = (carSpeed(i-6) + carSpeed(i-5) + carSpeed(i-4) + carSpeed(i-3) + carSpeed(i-2) + carSpeed(i-1))/6;
    ...
    if((carSpeed(i) > lastAverage*1.5 || carSpeed(i) < lastAverage*0.75) && lastAverage>2)
...
engineData = array2table([time eSpeed eSetPoint eP eI eD ePID eState carSpeed filteredCarSpeed]);
engineData = renamevars(engineData, ["Var1" "Var2" "Var3" "Var4" "Var5" "Var6" "Var7" "Var8" "Var9" "Var10"], ["Time" "Engine Speed" "Engine Speed Setpoint" "Engine P" "Engine I" "Engine D" "Engine PID" "Engine State" "Car Speed" "Filtered Car Speed"]);
engineVars = {'Engine State',{'Engine Speed', 'Engine Speed Setpoint'}, {'Engine P', 'Engine I','Engine D','Engine PID'}, {'Filtered Car Speed' 'Car Speed'}};
engineLoopTL = stackedplot(engineData, engineVars, 'XVariable','Time');
engineLoopTL.AxesProperties(2).YLimits = [-100 3800];
engineLoopTL.AxesProperties(4).YLimits = [0 35];
...
secondaryData = array2table([time sEncoder sEncoderTarget sControllerOutput sEncoderPID sLoadCellPID sLoadCellP sLoadCellI sLoadCellD PIDZeroMark  sState sClampForce clampForceTarget sMotorCurrent]);
secondaryData = renamevars(secondaryData, ["Var1" "Var2" "Var3" "Var4" "Var5" "Var6" "Var7" "Var8" "Var9" "Var10" "Var11" "Var12" "Var13" "Var14"], ["Time" "Secondary Encoder Ticks" "Secondary Encoder Ticks Target" "Secondary Controller Output" "sEncoderPID" "sLoadCellPID" "sLoadCellP" "sLoadCellI" "sLoadCellD" "PIDZeroMark" "Secondary State" "Secondary Force" "Target Force" "Motor Current"]);
secondaryVars = {'Secondary State', {'Secondary Encoder Ticks', 'Secondary Encoder Ticks Target'}, {'Secondary Controller Output', 'sEncoderPID', 'sLoadCellPID', 'PIDZeroMark'}, {'sLoadCellP', 'sLoadCellI', 'sLoadCellD', 'PIDZeroMark'}, {'Secondary Force', 'Target Force'}, 'Motor Current'};
...
elseif(pEncoder > 24148)
    ...
    ratio = 1 ./ sqrt((1/highRatio^2) - ratioPercent .* ((1/(highRatio^2) - 1/(lowRatio^2)) / (101-1)));
